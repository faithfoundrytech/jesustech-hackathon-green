...
export function EditTherapistModal({ open, setOpen, therapist }: EditTherapistModalProps) {
  const { updateTherapist, isUpdating, updateError, refetch } = useTherapists();
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            {updateError && (
              <Alert variant="destructive">
                <AlertDescription>
                  {updateError instanceof Error ? updateError.message : 'Failed to update therapist'}
                </AlertDescription>
              </Alert>
            )}
            <FormField
